...
class GradTTS(AbsTTS):
    ...
    def _forward(
        ...
        # ʹ��fastspeech����duration predictor��length regulator
        d_outs = self.duration_predictor(mu_x, x_mask)
    ...
    def inference(
        ...
        vocoder = HiFiGAN(h)
        vocoder.load_state_dict(torch.load("/home/lsh/content/espnet/espnet2/tts/gradtts/checkpts/hifigan.pt", map_location=lambda loc, storage: loc)['generator'])
        _ = vocoder.cuda().eval()
        ...
        write(f'/home/lsh/content/espnet/egs2/ljspeech/tts1/exp/tts_train_gradtts_raw_phn_tacotron_g2p_en_no_space/origin_vocoder/wav{audio_ids}.wav', 22050, audio)  # 指定保存文件夹
